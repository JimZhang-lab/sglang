...
def resolve_auto_parsers(server_args):
    """The config to launch with, with ``--reasoning-parser=auto`` /
    ``--tool-call-parser=auto`` resolved from the chat template.

    Performs a lightweight tokenizer load, so it runs once in engine init,
    before the scheduler subprocesses are spawned: they resolve their own
    config bags from the instance they are handed, which is the one returned
    here.
    """
    needs = tuple(
        attr
    ...
    return server_args.derive("template-detection", **detected)
